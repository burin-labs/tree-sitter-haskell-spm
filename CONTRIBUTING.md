# Contributing to `tree-sitter-haskell-spm`

## Scope

This repository is a packaging wrapper, not a product. It exists so Swift
projects can depend on the upstream Haskell tree-sitter grammar without running
...
[`tree-sitter-haskell`](https://github.com/tree-sitter/tree-sitter-haskell) tree-sitter grammar. The generated parser
(`Sources/TreeSitterHaskell/src/parser.c` and any `scanner.c`) is **vendored** so
...
You should see `T _tree_sitter_haskell`. If the grammar ships an
...
2. Copy `src/parser.c`, `src/scanner.c` (if present), and `src/tree_sitter/*.h`
   into `Sources/TreeSitterHaskell/src/`.
